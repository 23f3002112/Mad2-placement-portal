  ...
  <div class="mt-4">
    <div class="d-flex justify-content-between align-items-center mb-4">
      ...
      <button class="btn btn-primary" @click="triggerExport" :disabled="isExporting">
        ...
        {{ isExporting ? 'Starting Export...' : 'Export Applications (CSV)' }}
      </button>
    </div>
...
export default {
  ...
  methods: {
    async fetchExports() {
      try {
        const response = await api.get('/company/exports');
    ...
    async downloadFile(jobId) {
      try {
        const response = await api.get(`/company/exports/${jobId}/download`, {
          responseType: 'blob'
        });
        const url = window.URL.createObjectURL(new Blob([response.data]));
        const link = document.createElement('a');
        link.href = url;
        link.setAttribute('download', `export_company_${jobId}.csv`);
