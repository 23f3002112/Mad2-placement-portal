<template>
  <div class="card border-0 shadow-sm rounded-3 overflow-hidden" style="height: 75vh;">
    <div class="row g-0 h-100">
      <!-- Sidebar / Conversations List -->
      <div class="col-md-4 border-end h-100 d-flex flex-column bg-white">
        <div class="p-3 border-bottom bg-light">
          <h5 class="mb-0 fw-bold text-dark">Your Chats</h5>
        </div>
        <div class="overflow-auto flex-grow-1">
          <div v-if="conversations.length === 0" class="p-4 text-center text-muted">
            No chats yet. A company will message you if they shortlist you.
          </div>
          <div 
          ...
          >
            <div class="d-flex justify-content-between align-items-baseline mb-1">
              <h6 class="mb-0 fw-bold text-dark">{{ conv.company_name }}</h6>
      ...
      <div class="col-md-8 h-100 d-flex flex-column bg-white">
        <template v-if="activeConv">
          ...
          <div class="p-3 border-bottom bg-light d-flex justify-content-between align-items-center">
            <div>
              <h5 class="mb-0 fw-bold text-dark">{{ activeConv.company_name }}</h5>
              <small class="text-muted">Role: {{ activeConv.job_title }}</small>
            </div>
            <span class="badge bg-secondary rounded-pill px-3">{{ activeConv.status }}</span>
          </div>
        ...
        <div v-else class="h-100 d-flex flex-column align-items-center justify-content-center text-muted bg-light">
          ...
          <p>Read your messages from companies here.</p>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  methods: {
    ...
    async fetchMessages() {
      ...
      try {
        const response = await api.get(`/student/messages/${this.activeConv.application_id}`);
    ...
    async sendMessage() {
      ...
      try {
        const response = await api.post(`/student/messages/${this.activeConv.application_id}`, {
